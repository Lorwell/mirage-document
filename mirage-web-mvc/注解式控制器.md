...
| 返回值        | 描述                                                         |
| ------------- | ------------------------------------------------------------ |
| Future        | 所有注解方法的返回值都应该有`Future`对象包裹[kotlin的协程方法除外](Kotlin协程.md)，以下返回值表示的都是 `Future` 所包裹的类型。 |
...
针对http请求主体我们定义了`HttpMessageConverter`的方式来解析内容，默认情况下提供基于`jackson`的实现`JacksonHttpMessageConverter`，支持解析`application/json`,`application/*+json`请求或响应类型的内容，亦或者你可以通过`MirageWebMvcConfigurer`注册自定义的`HttpMessageConverter`
