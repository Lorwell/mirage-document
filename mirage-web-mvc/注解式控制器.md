...
| @ResponseBody | 响应注解方法注解，指方法返回值应绑定到web响应体上。支持带定义在类级别，方法级别未定义则继承类级别。响应的主体通过`HttpMessageConverter`来解析方法返回值，具体取决于响应的内容类型。 |

#### Jackson JSON

mirage 提供对 Jackson JSON库的支持，要将其与 `@ResponseBody`标识的控制器方法一起使用，它仅允许渲染`Object`的结果值。
